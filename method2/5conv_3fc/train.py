# -*- coding: utf-8 -*-
"""
Created on Wed Dec  6 21:30:55 2017
...
#数据集地址
path='E:/speaker_recognition_demo/method2/data/dataset_all_images/'
#模型保存地址
model_path='E:/speaker_recognition_demo/method2/5conv_3fc/model/model.ckpt'
...
def inference(input_tensor, train, regularizer):
    ...
    with tf.variable_scope('layer12-fc2'):
        fc2_weights = tf.get_variable("weight", [128, 64],
                                      initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    with tf.variable_scope('layer13-fc3'):
        fc3_weights = tf.get_variable("weight", [64, 8],
                                      initializer=tf.truncated_normal_initializer(stddev=0.1))
        if regularizer != None: tf.add_to_collection('losses', regularizer(fc3_weights))
        fc3_biases = tf.get_variable("bias", [8], initializer=tf.constant_initializer(0.1))
...
# batchSize 代表在做随机梯度下降时，使用批梯度的数量，即每次使用batchSize个数据来更新参数,1个iteration等于使用batchSize个样本训练一次,1个nEpoch等于使用所有样本训练一次
n_epoch=40                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       
batch_size=32
...
for epoch in range(n_epoch):
    ...
    print("   tra loss: %f" % (np.sum(train_loss)/ n_batch))
    print("   tra acc: %f" % (np.sum(train_acc)/ n_batch))
    ...
    print("   val loss: %f" % (np.sum(val_loss)/ n_batch))
    print("   val acc: %f" % (np.sum(val_acc)/ n_batch))
