...
#这些参数的含义不太明白还，这些数字代表的是什么含义，应该依据什么去设置。
    max_checkpoints = 3
    ...
    test_size = 2
#声纹识别，识别的是说话人是谁，所以应该把每个文件夹中的一部分图片用于训练，一部分用于测试。为什么要把部分文件夹用于训练，部分文件夹用于测试呢
    ...
    with tf.Session(config=config) as sess:
        ...
        while step <= args.max_step:
            ...
            if step % display_step == 0:
                ...
                traintxt=str(train_acc)          # data是前面运行出的数据，先将其转为字符串才能写入
               
                
                with open('F:\\speaker_recognition_demo\\method1\\result\\train_all_batchsize16.txt','a') as file_handle:
            ...
            if step % evaluate_step == 0:
                ...
                '''
                validationtxt=str(test_acc)          # data是前面运行出的数据，先将其转为字符串才能写入
                with open('F:\\speaker_recognition_demo\\method1\\result\\validation_all_batchsize16.txt','a') as file_handle:
...
def parse_arguments(argv):
    ...
    parser.add_argument('--max_step', type=int, help='Number of steps to run.', default=2000)
    parser.add_argument('--batch_size', type=int, help='batch size.', default=32)
    parser.add_argument('--CUDA_VISIBLE_DEVICES', type=str, help='CUDA VISIBLE DEVICES', default='9')
    parser.add_argument('--config_file', type=str, help='File containing the learning rate schedule', default='E:\\speaker_recognition_demo\\method1\\train_from_zero\\data\\config.txt')
    parser.add_argument('--model_name', type=str, help='', default='test')
    parser.add_argument('--logs_base_dir', type=str, help='Directory where to write event logs.', default='E:\\speaker_recognition_demo\\method1\\train_from_zero\\logs\\')
    parser.add_argument('--models_base_dir', type=str, help='Directory where to write trained models and checkpoints.', default='E:\\speaker_recognition_demo\method1\\train_from_zero\\models\\voice\\VCTK\\')
    ...
    parser.add_argument('--data_set', type=str, help="data set position.", default='E:\\speaker_recognition_demo\\method1\\dataset_all')
